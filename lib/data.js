...
function normalizeVehicleName(name = "") {
  ...
  const legacyCheryQName = ["q", "q3"].join("");
  return trimmed.toLowerCase() === legacyCheryQName ? "Chery Q" : trimmed;
}
...
export async function getVehicleMaterials() {
  ...
  const cleaned = data
    .map(item => ({
      ...item,
      Vehicle: normalizeVehicleName(item.Vehicle || item["Vehicle Name"] || item.vehicle || ""),
